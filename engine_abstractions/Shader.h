...
namespace rtre {
	...
	public:
		...
		inline void SetUniform(const char* name, Sphere sphere) {
		...
		inline void SetUniform(const char* name, Material mat) {
			std::string uniName = std::string(name) + ".";
			...
			glUniform1f(getUnifromID((uniName + "absorb").c_str()), mat.absorb);
		}
